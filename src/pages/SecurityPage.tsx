...
const SecuritySection = ({ icon: Icon, title, description, features }: { icon: any, title: string, description: string, features: string[] }) => (
  <div className="glass p-8 rounded-2xl border border-white/10">
    ...
    <p className="text-secondary-text mb-8 leading-relaxed">{description}</p>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {features.map((f, i) => (
        <div key={i} className="flex items-center space-x-2">
          <CheckCircle className="w-4 h-4 text-success" />
          <span className="text-sm text-primary-text">{f}</span>
        </div>
      ))}
    </div>
  </div>
  ...
  return (
    <div className="min-h-screen pt-24 pb-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      <div className="text-center mb-16">
        <h1 className="text-4xl font-bold text-primary-text mb-4">Security Model</h1>
      ...
      <div className="mt-24 p-12 rounded-3xl bg-gradient-to-br from-cyber-blue/20 to-teal/20 border border-white/10 text-center">
        <Shield className="w-16 h-16 text-teal mx-auto mb-6" />
        <h2 className="text-3xl font-bold text-primary-text mb-4">AI-Based Threat Monitoring</h2>
        ...
        <div className="inline-flex items-center space-x-2 text-teal font-bold uppercase tracking-widest text-sm">
