...
router.post('/file-return', authMiddleware, async (req, res) => {
  ...
  if (existing) return res.status(409).json({ success: false, message: `Return for AY ${ay} already filed` });
...
router.post('/calculate', (req, res) => {
  ...
  res.json({ success: true, calculation: { grossIncome: income, taxableIncome, tax: Math.round(tax), cess: Math.round(cess), totalTax: Math.round(totalTax), appliedDeductions, regime } });
});
